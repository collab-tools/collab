...
describe("AvatarList.jsx ", function() {
  ...
  it("render with no memeber", function() {
    expect(shallow(<AvatarList/>)).to.exist;
  });
  ...
  describe("with one member and rest prop empty", function(){
    ...
    it("first avatar follows all props", function() {
      let firstMember = this.customProps.members[0]
      expect(this.wrapper.find('UserAvatar').first().props().imgSrc).to.equal(firstMember.display_image)
      expect(this.wrapper.find('UserAvatar').first().props().displayName).to.equal(firstMember.display_name)
  ...
  describe("with one member and all props", function(){
    ...
    it("first avatar follows all props", function() {
      ...
      expect(this.wrapper.find('UserAvatar').first().props().displayName).to.equal(firstMember.display_name)
      expect(this.wrapper.find('UserAvatar').first().props().enableTooltip).to.equal(true)
      expect(this.wrapper.find('UserAvatar').first().props().isSquare).to.equal(this.customProps.isSquare)
      expect(this.wrapper.find('UserAvatar').first().props().memberColour).to.equal(this.customProps.colour?firstMember.colour: false)
    });
  })
  ...
  describe("with multiple members and all props", function(){
    ...
    it("last avatar follows all props", function() {
      let lastMember = this.customProps.members[this.customProps.members.length-1]
      expect(this.wrapper.find('UserAvatar').last().props().imgSrc).to.equal(lastMember.display_image)
      expect(this.wrapper.find('UserAvatar').last().props().displayName).to.equal(lastMember.display_name)
      expect(this.wrapper.find('UserAvatar').last().props().enableTooltip).to.equal(true)
      expect(this.wrapper.find('UserAvatar').last().props().isSquare).to.equal(this.customProps.isSquare)
      expect(this.wrapper.find('UserAvatar').last().props().memberColour).to.equal(this.customProps.colour?lastMember.colour: false)
    });
  })
